...
const SECTION_LOCK_KEYS = {
  access: 'doctor_access',
  clinical: 'doctor_clinical',
...
export default function AdminDoctorManagementPanel({ manageAdminEditLocks = false }) {
  ...
  const serverPermSignature = useMemo(() => {
    ...
    return (role.permissions ?? [])
      .filter((key) => allDoctorKeys.has(key))
      .slice()
      .sort()
      .join('|');
  }, [allDoctorKeys, roleMap, rolesQ.data, targetRoleKey]);
  ...
  const persistRolePermissions = async () => {
    if (!targetRoleKey) {
      toast.error('Role "doctor" is not available in this hospital.');
      return;
    }
    const role = roleMap.get(targetRoleKey);
    if (!role) {
    ...
    if (serverPermSignature == null) {
      toast.error('Doctor permissions are still loading. Try again in a moment.');
      return;
    }

    const selectedKeys = [...(selectedByRole[targetRoleKey] ?? new Set())];
    const existingCount = (role.permissions ?? []).filter((key) => allDoctorKeys.has(key)).length;
    ...
    const keepOther = existing.filter((key) => !allDoctorKeys.has(key));
    ...
    try {
      ...
      setIsDirty(false);
      toast.success('Doctor permissions saved. Doctor screens refresh within a few seconds.');
      try {
        const bump = String(Date.now());
        localStorage.setItem('hms:doctor-permissions-bump', bump);
        window.dispatchEvent(new Event('hms:doctor-permissions-bump'));
      } catch {
        /* ignore */
      }
      rolesQ.refetch();
  ...
  return (
    <div className="aos-page">
      <div className="aos-page__intro">
        <div>
          <h2 className="admin-card__title">Doctor settings</h2>
        </div>
      </div>
      ...
      >
        <div className="aos-section-tabs" role="tablist" aria-label="Doctor settings sections">
        ...
        {activeSection === 'access' ? (
          <SectionCard
            ...
            tone="blue"
            defaultOpen
            action={lockToggle('doctor_access')}
            locked={!canEdit('doctor_access')}
          >
            <PermissionModuleList
              modules={['Appointments & Dashboard', 'Patients', 'Notifications']}
              targetRoleKey={targetRoleKey}
              selectedByRole={selectedByRole}
              togglePermission={togglePermission}
              disabled={!canEdit('doctor_access')}
            />
          </SectionCard>
        ) : null}
        ...
        {activeSection === 'clinical' ? (
          <SectionCard
            ...
            tone="teal"
            defaultOpen
            action={lockToggle('doctor_clinical')}
            locked={!canEdit('doctor_clinical')}
          >
            <PermissionModuleList
              modules={['Consultations', 'Prescriptions', 'Lab Tests']}
              targetRoleKey={targetRoleKey}
              selectedByRole={selectedByRole}
              togglePermission={togglePermission}
              disabled={!canEdit('doctor_clinical')}
            />
          </SectionCard>
        ) : null}
        ...
        {activeSection === 'profile' ? (
          <SectionCard
            ...
            tone="indigo"
            defaultOpen
            action={lockToggle('doctor_profile')}
            locked={!canEdit('doctor_profile')}
          >
            <PermissionModuleList
              modules={['Profile']}
              targetRoleKey={targetRoleKey}
              selectedByRole={selectedByRole}
              togglePermission={togglePermission}
              disabled={!canEdit('doctor_profile')}
            />
          </SectionCard>
        ) : null}
            ...
            title={
              !canEdit(activeSectionLockKey)
                ...
                : targetRoleKey
                  ...
                  : 'Doctor role not available for permission control'
            }
          >
            <Save size={16} />
            {saveMut.isPending ? 'Saving…' : 'Save Doctor Permissions'}
          </Button>
        </div>
      </QueryFeedback>
    </div>
