...
const TARGET_ROLE_KEY = 'lab_technician';
const SECTION_TABS = [
...
export default function AdminLabManagementPanel({ manageAdminEditLocks = false }) {
  ...
  const serverPermSignature = useMemo(() => {
    ...
    return (role.permissions ?? [])
      .filter((key) => allLabKeys.has(key))
      .slice()
      .sort()
      .join('|');
  }, [allLabKeys, roleMap, rolesQ.data, targetRoleKey]);
  ...
  const persistRolePermissions = async () => {
    if (!targetRoleKey) {
      toast.error('Role "lab_technician" is not available in this hospital.');
      return;
    }
    const role = roleMap.get(targetRoleKey);
    if (!role) {
    ...
    if (serverPermSignature == null) {
      toast.error('LAB permissions are still loading. Try again in a moment.');
      return;
    }

    const selectedKeys = [...(selectedByRole[targetRoleKey] ?? new Set())];
    const existingCount = (role.permissions ?? []).filter((key) => allLabKeys.has(key)).length;
    ...
    const keepOther = existing.filter((key) => !allLabKeys.has(key));
    ...
    try {
      ...
      setIsDirty(false);
      toast.success('LAB permissions saved. Lab screens refresh within a few seconds.');
      try {
        const bump = String(Date.now());
        localStorage.setItem('hms:lab-permissions-bump', bump);
        window.dispatchEvent(new Event('hms:lab-permissions-bump'));
      } catch {
        /* ignore */
      }
      rolesQ.refetch();
  ...
  return (
    <div className="aos-page">
      <div className="aos-page__intro">
        <div>
          <h2 className="admin-card__title">LAB settings</h2>
        </div>
      </div>
      ...
      >
        <div className="aos-section-tabs" role="tablist" aria-label="LAB settings sections">
        ...
        {activeSection === 'access' ? (
          <SectionCard
            ...
            tone="blue"
            defaultOpen
            action={lockToggle('lab_access')}
            locked={!canEdit('lab_access')}
          >
            <PermissionModuleList
              modules={['Orders & Reports', 'Notifications']}
              targetRoleKey={targetRoleKey}
              selectedByRole={selectedByRole}
              togglePermission={togglePermission}
              disabled={!canEdit('lab_access')}
            />
          </SectionCard>
        ) : null}
        ...
        {activeSection === 'results' ? (
          <SectionCard
            ...
            tone="teal"
            defaultOpen
            action={lockToggle('lab_results')}
            locked={!canEdit('lab_results')}
          >
            <PermissionModuleList
              modules={['Results Workflow']}
              targetRoleKey={targetRoleKey}
              selectedByRole={selectedByRole}
              togglePermission={togglePermission}
              disabled={!canEdit('lab_results')}
            />
          </SectionCard>
        ) : null}
        ...
        {activeSection === 'profile' ? (
          <SectionCard
            ...
            tone="indigo"
            defaultOpen
            action={lockToggle('lab_profile')}
            locked={!canEdit('lab_profile')}
          >
            <PermissionModuleList
              modules={['Profile']}
              targetRoleKey={targetRoleKey}
